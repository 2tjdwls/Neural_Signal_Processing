...
# Read EEG_mat file
os.chdir('C:/Users/HBFLAB/Desktop/LSJ')
cwd = os.getcwd()
print('$ Current working dir:   ', cwd)
...
type(epochs_dict[key[0]])

# MNE epoch plot
epochs_dict[key[0]].plot(scalings=20, n_channels=60)
...
epochs_dict[key[A]].set_montage(montage)

"""
# Check that the locations of EEG electrodes is correct with respect to MRI
mne.viz.plot_alignment(
    epochs_dict[key[A]].info, eeg=['original', 'projected'], trans=trans,
    show_axes=True, mri_fiducials=True, dig='fiducials', subject=subject, bem=bem)

# Check that the locations of EEG electrodes is correct with respect to MRI
mne.viz.plot_alignment(
    epochs_dict[key[A]].info, 
    src=src, 
    eeg='projected', 
    trans=trans,
    show_axes=True,
...
fwd = mne.make_forward_solution(epochs_dict[key[A]].info, trans=trans, src=src,
                                bem=bem, eeg=True, mindist=5.0, n_jobs=1)

# Compute noise covariance during baseline
noise_cov = mne.compute_covariance(epochs_dict[key[A]], tmax=0, method='auto')

# Inverse operator
inv = mne.minimum_norm.make_inverse_operator(
      info=epochs_dict[key[A]].info, 
      forward=fwd, noise_cov=noise_cov
      )

# Plot noise covariance
mne.viz.plot_cov(noise_cov,info=epochs_dict[key[A]].info)
...
mean_stcs.save(key[A], overwrite=True)
...
labels = mne.read_labels_from_annot(subject,
                                    parc=parc, hemi='both',
                                    surf_name='inflated',
                                    regexp='Brodmann.', # Only use 41 brodmann areas
...
tc = mne.extract_label_time_course(stcs, labels, src, mode='auto')
tc = np.array(tc)

# Save source data to MAT file

source_dict[key[A]] = tc

from scipy.io import savemat

for A in range(6) :
    print(source_dict[key[A]].shape)
    X = source_dict[key[A]]
    
    savemat(key[A]+'.mat', {'data' : X})
